...
procedure borrarEmpleado(var arch: archivo_empleados; nroEmp: integer);
var
	e: empleado;
	encontre: boolean;
	pos: integer;
begin
	...
	if encontre then begin
		// Me guardo la pos del empleado que tengo que borrar.
		seek(arch, filePos(arch) - 1);
		pos:= filePos(arch);
		
		// Me muevo al final para obtener el ultimo empleado
		seek(arch, fileSize(arch) - 1);
		read(arch, e);
		seek(arch, filePos(arch) - 1);
		truncate(arch); // Pongo EOF en esta pos.
		
		seek(arch, pos); // Vuelvo a donde tengo que borrar.
		
		write(arch, e);
	end
	else
		writeln('  [!] No se encontro ningun empleado con ese nro...');
end;
		...
		if i = 1 then begin
			...
			while j <> 0 do begin
				...
				if j = 1 then begin
					...
					write('Ingrese nro del empleado a borrar: ');
					readln(nroEmp);
					
					borrarEmpleado(arch, nroEmp);
